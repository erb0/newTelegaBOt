...
async function searchPayment(User, ctx) {
  ...
  let table = `👤 ( ${user.data.searchValue} )\nСписок последних оплат:
  ...
  try {
    ...
    if (data && data.length > 0) {
      ...
      await ctx.telegram.editMessageText(
        chatId, // Идентификатор чата, куда было отправлено сообщение
        user.data.sentMessage, // Идентификатор изменяемого сообщения
        null, // Если вы не хотите менять местоположение сообщения (по умолчанию null)
        table,
        menu // Новый текст сообщения
      )
    } else {
...
async function searchCheap(User, ctx) {
  ...
  try {
    const waterMeters = await getWaterMeters(user.data.searchValue)
    const formattedResult = await formatResult(waterMeters)

    ctx.replyWithHTML(formattedResult, menu) // Отправляем сообщение
  } catch (error) {
    console.error('Ошибка при выполнении запроса:', error)
  }
}
